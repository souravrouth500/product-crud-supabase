...
// export const updateProduct = async ({ values, img }: { values: RequestBody, img: File }) => {
//     console.log(values);
//     if ('id' in values) {
//         console.log("updateAPI",values);
        
//         const { data, error } = await supabase.from('products').update(values).eq('id', values.id).select()
//         if (error) {
//             return error
//         }
//         return data
...
export const updateProduct = async ({ values, img }: { values: RequestBody, img: File }) => {
    ...
    if ('id' in values) {
        ...
        const updateBody = { ...values, image_url: fullPath }
        
        const { data, error } = await supabase.from('products').update(updateBody).eq('id', values.id).select()
        if (error) {
            return error
        }
        return data
...
export const removeProduct = async (id: string) => {
    const { error: fileRes } = await supabase.storage.from('logo').remove([`${id}.jpg`])

    let productRes;

    if (!fileRes) {
        productRes = await supabase.from('products').delete().eq('id', id)
    }
